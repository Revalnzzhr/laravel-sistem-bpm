...
<div class="d-flex flex-column min-vh-100 p-5 pt-0">
    ...
    <div class="shadow p-5 m-5 mt-3 bg-white rounded">
        ...
        <form method="POST" action="{{ route('berita.update', $berita->ber_id) }}" enctype="multipart/form-data">
            @csrf
            @method('POST')

            <div class="form-group mb-3">
                <label for="ber_judul" class="form-label fw-bold">Judul Berita</label>
                <input type="text" name="ber_judul" id="ber_judul" class="form-control" value="{{ $berita->ber_judul }}" required>
            </div>
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="date" name="ber_tgl" id="ber_tgl" class="form-control"  max="{{ date('Y-m-d') }}" value="{{ old('ber_tgl', \Carbon\Carbon::parse($berita->ber_tgl)->format('Y-m-d')) }}" required>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group mb-3">
                        <label for="ber_penulis" class="form-label fw-bold">Penulis</label>
                        <input type="text" name="ber_penulis" id="ber_penulis" class="form-control" value="{{ $berita->ber_penulis }}" required>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <textarea name="ber_isi" id="ber_isi" class="form-control" rows="5">{{ $berita->ber_isi }}</textarea>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-4 col-md-6 mb-4">
                    ...
                    <div class="border p-2 text-center" style="min-width: 150px; min-height: 200px;">
                        ...
                        <img id="preview{{ $index }}" src="{{ asset('storage/' . $berita["ber_foto$index"]) }}" class="img-fluid img-thumbnail" alt="Foto {{ $index }}" style="max-height: 200px;">
                        @else
...
document.querySelector('form').addEventListener('submit', function (e) {
    ...
    // Validasi ber_foto1 wajib diisi, kecuali sudah ada gambar lama
    ...
    if (!fotoInput1.files.length && (!preview1.src || preview1.src === '')) {
        e.preventDefault();
        Swal.fire({
            icon: 'error',
            title: 'Oops...',
    ...
    // Loop untuk memeriksa semua input file
    document.querySelectorAll('input[type="file"]').forEach((input) => {
        const preview = document.getElementById(`preview_${input.id}`); // Ambil elemen preview sesuai ID

        // Jika input kosong tetapi ada gambar lama, maka lolos validasi
        if (!input.files.length && preview && preview.src && preview.src !== '') {
            return;
        }
        ...
        if (input.files.length) {
            ...
            if (!allowedExtensions.includes(file.type)) {
                ...
                Swal.fire({
                    ...
                    text: `File pada ${input.name} harus berupa gambar (JPG, PNG, atau GIF).`,
                });
