...
<div class="container mt-4 mb-5">
    ...
    <ul class="nav nav-tabs mb-3" id="jenisKegiatanTabs" role="tablist">
        @foreach($jenisKegiatan as $jenis)
            <li class="keg-item" role="presentation">
            ...
            <div class="accordion" id="accordion-{{ $jenis->jkg_id }}">
                ...
                <div class="accordion-item">
                    ...
                    <button class="accordion-button collapsed" type="button" 
                        ...
                        data-bs-target="#collapse-{{ $jenis->jkg_id }}-{{ $tahun }}" 
                        aria-expanded="false" 
                        aria-controls="collapse-{{ $jenis->jkg_id }}-{{ $tahun }}">
                        {{ $tahun }}
                    </button>


                    </h2>
                    <div id="collapse-{{ $jenis->jkg_id }}-{{ $tahun }}" 
                        class="accordion-collapse collapse" 
                        aria-labelledby="heading-{{ $jenis->jkg_id }}-{{ $tahun }}">
    ...
    accordionButtons.forEach(function (button) {
        button.addEventListener('click', function () {
            ...
            if (!element.classList.contains('show')) {
                ...
                element.classList.add('show'); // Show the accordion content
               
                
            } else{
                console.log("jalan")
                element.classList.remove('show'); // Hide the accordion content
                element.classList.add('collapse'); // Add 'collapse' to hide it
